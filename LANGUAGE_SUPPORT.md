...
| C          | `.c`, `.h`    | tree-sitter-c          | function, type (struct/enum/typedef), constant | -             | `//` and `/* */` comments | No classes; `.h` files default to C (use `.hpp` for C++) |
| C++        | `.cpp`, `.cc`, `.cxx`, `.hpp`, `.hh`, `.hxx` | tree-sitter-cpp | function, class, method, type (struct/enum/namespace), constant | - | `//` and `/* */` comments | Methods inside classes/structs/namespaces; `enum class` supported |
